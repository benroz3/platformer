...
class Player(pygame.sprite.Sprite):
    # More characters in assets/MainCharacters
...
class Fire(Object):
    ...
    def __init__(self, x, y, width, height):
        ...
        self.fire_images = load_sprite_sheets("Traps", 'Fire', width, height)
        self.image = self.fire_images['off'][0]
        self.mask = pygame.mask.from_surface(self.image)
        self.animation_count = 0
    ...
    def loop(self):
        sprites = self.fire_images[self.animation_name]
        sprite_index = (self.animation_count //
                        self.ANIMATION_DELAY) % len(sprites)
...
def main(window):
    ...
    # More background options inside assets/Background
    ...
    # Generate terrain
    fire = Fire(block_size * (4 + 1/3), HEIGHT - block_size - 64, 16, 32)
    fire.on()
    floor = [Block(i * block_size, HEIGHT - block_size, block_size)
             for i in range(-WIDTH // block_size, (WIDTH * 2) // block_size)]
    mountain = [Block(block_size * 6, HEIGHT - block_size * 2, block_size), Block(block_size * 7,
                                                                                  HEIGHT - block_size * 2, block_size), Block(block_size * 7, HEIGHT - block_size * 3, block_size)]
    objects = [*floor, Block(0, HEIGHT - block_size * 2, block_size),
               Block(block_size * 3, HEIGHT - block_size * 4, block_size), Block(block_size * 4,
                                                                                 HEIGHT - block_size * 4, block_size), fire, *mountain,
